...
/**
 * @author pengchenxue
 * 有些操作是不支持margin用户来访问的，但是你让我在大量接口里面都写一段
...
@Aspect
@Order(1)
public class MarginValidateAspect {
    ...
    @Pointcut("@annotation(com.binance.account.aop.MarginValidate)")//指向自定义注解路径
    ...
    public void before(JoinPoint joinPoint) throws Throwable {
        ...
        MarginValidate marginValidate = method.getAnnotation(MarginValidate.class);
        if (null == marginValidate) {
            log.warn("MarginValidateAspect.before：marginValidate=null");
            return;
        }
        Long userId =null;
        if(StringUtils.isAllBlank(marginValidate.email(),marginValidate.userId())){
            userId=getUserIdForCommon(joinPoint);
        }else{
            userId= getUserId(joinPoint, marginValidate);
        }
        if (null == userId) {
            log.warn("MarginValidateAspect.before：userId=null");
            return;
        }
        Boolean isMarginUser = userCommonValidateService.isMarginUserOrIsolatedMargin(userId);
        if (isMarginUser) {
    ...
    private Long getUserId(JoinPoint joinPoint, MarginValidate marginValidate) {
        Long userId = null;
        if (StringUtils.isNotBlank(marginValidate.userId())) {
            userId = fetchUseridFromExpress(joinPoint, marginValidate.userId());
        }
        if (StringUtils.isNotBlank(marginValidate.email())) {
            String email = fetchEmailFromExpress(joinPoint, marginValidate.email());
            User user = this.userMapper.queryByEmail(email);
            if (null != user) {
                userId = user.getUserId();
            }
