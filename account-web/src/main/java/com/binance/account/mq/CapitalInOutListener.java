...
public class CapitalInOutListener {
	...
	private static final String QUEUE_NAME = "address_audit_notify";
	private static final String ROUTING_KEY = "address.audit.notify";
	private static final String EXCHANGE_NAME = "address.audit.notify.exchange";
			...
			if (StringUtils.equalsIgnoreCase(PNK_WEB, sysType)) {
				...
				switch (optType) {
					case OPT_TYPE_CHARGE_CRYPTO:
						// coin sourceAddress
						Boolean isRiskCheckDeposit = StringUtils.isEmpty(sysConfigVarCacheService.getValue("risk_chainalysis_check_deposit")) ? Boolean.TRUE : Boolean.valueOf(sysConfigVarCacheService.getValue("risk_chainalysis_check_deposit"));
						if (isRiskCheckDeposit
							...
							String isFiatStr = StringUtils.isEmpty(sysConfigVarCacheService.getValue("isFiat")) ? "true" : sysConfigVarCacheService.getValue("isFiat");
