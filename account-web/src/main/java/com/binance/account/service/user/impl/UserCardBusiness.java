...
@Service
@Monitored
public class UserCardBusiness implements IUserCard {
    ...
    public APIResponse<CreateCardAccountResponse> createCardAccount(APIRequest<CreateCardAccountRequest> request) throws Exception {
        final CreateCardAccountRequest requestBody = request.getBody();
        log.info("Get creating card account request with uid-{}", requestBody.getUserId());
        //校验并且获取主账户信息  ,rootuser这里解释为主账户
        Pair<User, UserInfo> rootTuple = checkAndGetUserByIdForCardVersion(requestBody.getUserId(),null);
        //获取主账号相关信息
        User rootUser = rootTuple.getLeft();
        UserInfo rootUserInfo = rootTuple.getRight();
        if(null!=rootUserInfo.getCardUserId()){
            CreateCardAccountResponse createCardAccountResponse = new CreateCardAccountResponse();
            createCardAccountResponse.setRootUserId(rootUserInfo.getUserId());
            createCardAccountResponse.setRootTradingAccount(rootUserInfo.getTradingAccount());
            UserInfo oldCardUserInfo = userInfoMapper.selectByPrimaryKey(rootUserInfo.getCardUserId());
            if (null == oldCardUserInfo) {
                throw new BusinessException(GeneralCode.USER_NOT_EXIST);
            }
            createCardAccountResponse.setCardUserId(oldCardUserInfo.getUserId());
            createCardAccountResponse.setCardTradingAccount(oldCardUserInfo.getTradingAccount());
            log.info("old card account for uid-{} succeed with result-{}", requestBody.getUserId(), createCardAccountResponse);
            return APIResponse.getOKJsonResult(createCardAccountResponse);
        }

        //开始创建mining账号相关信息
        String vritualEmail = createVirtualEmailForCard(rootUser.getEmail(), rootUser.getUserId());
        User cardUser = ((UserCardBusiness) AopContext.currentProxy()).createCardUser(vritualEmail);

        // 创建mining账号Security信息
        ((UserCardBusiness) AopContext.currentProxy()).createCardUserSecurity(cardUser.getUserId(), cardUser.getEmail());

        // 创建mining账号info信息
        UserInfo cardUserInfo = ((UserCardBusiness) AopContext.currentProxy()).createCardUserInfo(rootUserInfo, cardUser.getUserId());

        // 更新root表中的mininguserid （这步操作是幂等的）
        rootUserInfo.setCardUserId(cardUser.getUserId());
        userInfoMapper.updateByPrimaryKeySelective(rootUserInfo);

        //更新主账户的状态(这步操作也是幂等的),这是用来标明这个账户是否拥有card账户
        rootUser.setStatus(BitUtils.enable(rootUser.getStatus(), AccountCommonConstant.USER_IS_EXIST_CARD_ACCOUNT));
        userMapper.updateByEmailSelective(rootUser);
        // 创建card交易账户
        //这里不需要考虑创建账户失败的case，因为我在postAccount方法里面已经处理了，出错直接抛出异常，所以上层调用不要再判断了
        Long cardTradingAccount = matchboxApiClient.postAccount(cardUserInfo, MatchBoxAccountTypeEnum.C2C);
        cardUserInfo.setTradingAccount(cardTradingAccount);
        this.userInfoMapper.updateByPrimaryKeySelective(cardUserInfo);
        UserTradingAccount userTradingAccount = new UserTradingAccount();// 插入交易账户索引
        userTradingAccount.setTradingAccount(cardTradingAccount);
        userTradingAccount.setUserId(cardUserInfo.getUserId());
        this.userTradingAccountMapper.insert(userTradingAccount);// 交易账户索引 激活时创建交易账户
        log.info("UserCardBusiness.postAccount insert:{}", JSON.toJSONString(userTradingAccount));
        //禁用交易
        // 2.交易禁用-调用撮合引擎
        if (null != cardUserInfo && null != cardUserInfo.getTradingAccount() && cardUserInfo.getTradingAccount() > 0) {
            accountApi.setTradingAccount(cardUserInfo.getTradingAccount(), false, true, true);
            log.info("UserCardBusiness.disable tradingAccount done, userId:{}, tradingAccountId:{}", cardUserInfo.getUserId(),
                    cardUserInfo.getTradingAccount());
        }
        ...
        AsyncTaskExecutor.execute(() -> {
            try {
                // 加入推荐记录表
                insertToAgentLog(requestBody.getUserId(), cardUserInfo.getUserId(), cardUser.getEmail());
            } catch (Exception e) {
                log.error("insertToAgentLog exception", e);
            }
            try {
                userCommonBusiness.insertInfoRootUserIndex(requestBody.getUserId(),cardUserInfo.getUserId(), UserTypeEnum.CARD.name());
            }catch (Exception e){
                log.error("insertInfoRootUserIndex card exception", e);
            }
        });
        CreateCardAccountResponse createCardUserResponse = new CreateCardAccountResponse();
        createCardUserResponse.setRootUserId(rootUserInfo.getUserId());
        createCardUserResponse.setRootTradingAccount(rootUserInfo.getTradingAccount());
        createCardUserResponse.setCardUserId(cardUserInfo.getUserId());
        createCardUserResponse.setCardTradingAccount(cardTradingAccount);

        log.info("Create card account for uid-{} succeed with result-{}", requestBody.getUserId(), createCardUserResponse);

        return APIResponse.getOKJsonResult(createCardUserResponse);
    }
     ...
     */
    protected Pair<User, UserInfo> checkAndGetUserByIdForCardVersion(Long userId,Long parentUserId) throws Exception {
        if (null == userId) {
        ...
        //当前账号不能是card 账号
        if (BitUtils.isEnable(rootUser.getStatus(), AccountCommonConstant.USER_IS_CARD_USER)) {
            throw new BusinessException(AccountErrorCode.CARD_ACOUNT_CANNOT_CREATE_CARD_ACCOUNT);
        }
        //当前账号不能是子账号
        if (BitUtils.isEnable(rootUser.getStatus(), AccountCommonConstant.USER_IS_SUBUSER)) {
            throw new BusinessException(AccountErrorCode.SUB_ACOUNT_CANNOT_CREATE_CARD_ACCOUNT);
        }
     ...
     * 创建一个虚拟邮箱（幂等）
     */
    protected String createVirtualEmailForCard(String email, Long userId) {
        String[] emailArray = email.split("@");
        String virtualEmail = emailArray[0] + "_" + String.valueOf(userId)+ "_card@" + emailArray[1];
        return virtualEmail;
    }
     ...
     */
    @Transactional(value = DefaultDB.TRANSACTION, propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
    protected User createCardUser(final String vritualEmail) throws NoSuchAlgorithmException {
        ...
        status = BitUtils.enable(status, AccountCommonConstant.USER_IS_CARD_USER);
        //禁止登录
     ...
     */
    @Transactional(value = DefaultDB.TRANSACTION, propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
    protected void createCardUserSecurity(final Long userId, final String userEmail) {
     ...
     */
    @Transactional(value = DefaultDB.TRANSACTION, propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
    protected UserInfo createCardUserInfo(UserInfo rootUserInfo, Long cardUserId) {
        //逻辑很简单，从主账号的userinfo把信息都copy过来就完事了
        UserInfo cardUserInfo = new UserInfo();
        cardUserInfo.setUserId(cardUserId);
        // 被推荐人返佣比例
        cardUserInfo.setReferralRewardRatio(rootUserInfo.getReferralRewardRatio());
        // 经纪人返佣比例
        cardUserInfo.setAgentRewardRatio(rootUserInfo.getAgentRewardRatio());
        // 用户交易账号 激活时创建
        cardUserInfo.setTradingAccount(null);
        // 被动方手续费
        cardUserInfo.setMakerCommission(rootUserInfo.getMakerCommission());
        // 主动方手续费
        cardUserInfo.setTakerCommission(rootUserInfo.getTakerCommission());
        // 买方交易手续费
        cardUserInfo.setBuyerCommission(rootUserInfo.getBuyerCommission());
        // 卖方交易手续费
        cardUserInfo.setSellerCommission(rootUserInfo.getSellerCommission());
        // 单日最大出金总金额
        cardUserInfo.setDailyWithdrawCap(rootUserInfo.getDailyWithdrawCap());
        // 单日最大出金次数
        cardUserInfo.setDailyWithdrawCountLimit(rootUserInfo.getDailyWithdrawCountLimit());
        // 免审核额度
        cardUserInfo.setAutoWithdrawAuditThreshold(rootUserInfo.getAutoWithdrawAuditThreshold());
        // 交易等级
        cardUserInfo.setTradeLevel(rootUserInfo.getTradeLevel());
        // 新返佣比例
        cardUserInfo.setReferralRewardRatio(rootUserInfo.getReferralRewardRatio());
        cardUserInfo.setNickName("");
        cardUserInfo.setRemark("");
        cardUserInfo.setTrackSource(rootUserInfo.getTrackSource());
        cardUserInfo.setInsertTime(DateUtils.getNewDate());
        cardUserInfo.setUpdateTime(DateUtils.getNewDate());
        cardUserInfo.setAccountType(UserTypeEnum.CARD.name());
        // 推荐人
        cardUserInfo.setAgentId(rootUserInfo.getAgentId());
        // 返佣开关关闭的话，无视推荐人
        ...
        if (rootUserInfo.getAgentId() == null) {
            ...
            cardUserInfo.setAgentId(defaultAgentId);
        }
        log.info("register:插入userInfo信息");
        // 插入用户信息
        userInfoMapper.insert(cardUserInfo);
        return cardUserInfo;
    }
