...
public interface UserWithdrawPropertyApi {
    ...
    @PostMapping("/lock")
    APIResponse<UserWithdrawLockResponse> lock(@RequestBody APIRequest<UserWithdrawLockRequest> request) throws Exception;

    @ApiOperation(notes = "解锁提现额度", nickname = "unlock withdraw", value = "解锁提现额度")
    @PostMapping("/unlock")
    APIResponse<UserWithdrawLockResponse> unlock(@RequestBody APIRequest<UserWithdrawLockRequest> request) throws Exception;
