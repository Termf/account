...
public interface UserDeviceApi {
    ...
    APIResponse<Map<String, Object>> compareDeviceV2(@RequestBody APIRequest<CompareDeviceRequestV2> request);
    ...
    @ApiOperation(notes = "判断当前ip是否是新设备的ip", nickname = "checkNewDeviceIp", value = "判断当前ip是否是新设备的ip")
    @PostMapping("/checkNewDeviceIp")
    APIResponse<CheckNewDeviceIpResponse> checkNewDeviceIp(@RequestBody APIRequest<CheckNewDeviceIpRequest> request);


    @ApiOperation(notes = "新增设备指纹在扫码登录阶段", nickname = "新增设备指纹在扫码登录阶段", value = "新增设备指纹在扫码登录阶段")
    @PostMapping("/addDeviceForQRCodeLogin")
    APIResponse<AddUserDeviceForQRCodeLoginResponse> addDeviceForQRCodeLogin(@RequestBody APIRequest<AddUserDeviceForQRCodeLoginRequest> request);
