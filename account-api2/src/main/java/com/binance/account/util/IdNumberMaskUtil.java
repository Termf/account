...
public class IdNumberMaskUtil {
	 ...
	 * @param idNumber
	 */
	public static String getIdNumberMarkV2(String idNumber) {
		String result;
		if (StringUtils.isBlank(idNumber)) {
			result = "";
		} else {
			...
			if (idNumber.length() > 4) {
				tempNumber.append(idNumber.substring(0, 2)).append("**********")
						.append(idNumber.substring(idNumber.length() - 2, idNumber.length()));
			} else {
				tempNumber.append(ID_NUMBER_MARK).append(idNumber);
